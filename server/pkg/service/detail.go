...
func (s *DetailService) get15DaysKillStatisticsHistoricData(cog types.Cog) ([]types.CreatureKillStatistic, error) {
	var historicCreatureKillStatistics []types.CreatureKillStatistic
	filters := make([]types.Filter, 3)
	...
	for _, creature := range cog.Creatures {
		filters[0] = types.Filter{Name: "creature_name", Operand: constants.Equal, Value: creature.Name}

		results, err := s.killStatisticRepository.Find(types.Criteria{Filters: filters})
		...
		for _, result := range results {
			historicCreatureKillStatistics = append(historicCreatureKillStatistics, types.CreatureKillStatistic{
				Name:          result.CreatureName,
				DropRate:      result.DropRate,
				KillStatistic: result.AmountKilled,
				CreatedAt:     result.CreatedAt,
	...
	if len(historicCreatureKillStatistics) == 0 {
		historicCreatureKillStatistics = make([]types.CreatureKillStatistic, 0)
	}

	return historicCreatureKillStatistics, nil
}
